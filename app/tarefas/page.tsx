...
export default function TarefasPageAtualizada() {
  ...
  async function loadTasks() {
    ...
    // (no-store para evitar cache agressiva em alguns browsers/PWA)
    ...
    if (!meRes.ok) {
      ...
      setTarefas([]);
      setLoading(false);
      setMsg("Não foi possível obter dados do utilizador (/api/me).");
      return;
    }

    const me = await meRes.json();

    // Fallback robusto: se por algum motivo o /api/me vier sem familyId (cache/edge),
    // tenta usar o último familyId guardado ou a variável de ambiente.
    const storedFamilyId =
      typeof window !== "undefined" ? window.localStorage.getItem("familyId") : null;
    // Use familyId do /api/me, depois do localStorage e por último da variável de ambiente
  ...
  // Mantém a lista sincronizada entre dispositivos (ex.: marido/mulher)
  // via Realtime; se não estiver ativo, não quebra – apenas não atualiza automaticamente.
  useEffect(() => {
    if (!familyId) return;
    const channel = supabase
      ...
      .on(
        ...
        () => {
          // Recarrega quando houver mudanças (INSERT/UPDATE/DELETE)
          loadTasks();
        }
      )
      .subscribe();
